% -*- mode: cweb -*-
%cweb has its own \ifpdf, so undef it
\let\ifpdf=\relax
\input eplain
...
I just want to be able to generate AIGs that correspond to bitfunc PCODE
operations, using a Makefile.

@d set_bool_byte(bv) for (int i = 1; i < BITS_IN_BYTE; i++) (bv)->data[i] = LITERAL_FALSE;
...
@ To generate two argument AIGs, we use this:
