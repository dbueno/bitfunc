...
static int maxDegree(fibheap *h);
...
static int maxDegree(fibheap *h)
{
  assert(h->num != 0);
  double n = h->num * 1.f;
  int res = (int) (log(n) / log((1.f + sqrt(5.f)) / 2.f));
  return res;
}
