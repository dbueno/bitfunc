...
static unsigned AlarmInterval = 1, /* seconds */
...
static char *optDescription[] = {
  ...
  "Learned clause GC strategy: none, minisat (default), glucose.",
...
static char *strpuniq(const char *str, char *strs[])
{
  vector v;
  vectorInit(&v, 2);
  for (uintptr_t i = 0; strs[i]; i++) vectorPush(&v, strs[i]);
  for (uintptr_t i = 0; str[i]; i++) {
    for (uintptr_t j = 0; j < v.size;) {
      char *strj = v.data[j];
      if (str[i] != strj[i]) vectorPopAt(&v, j);
      else j++;
    }
  }
...
int main(int argc, char **argv)
{
  ...
  while (1) {
    ...
    case 0:
      ...
      if (longOptions[optidx].has_arg == required_argument ||
        ...
        if (optarg) {
          ...
          fprintf(stderr, "error: bad boolean option\n");
          usage(argc, argv);
          exit(1);
        }
    ...
    case 'j':
      conf->maxJailDecisionLevel = (unsigned) strtol((char *) optarg, NULL, 10);
      fprintf(stderr, "c maxJailDecisionLevel = %u\n", conf->maxJailDecisionLevel);
      break;
  ...
  parseDimacsCnf(argv[optind], func);
  /* funcsatDebug(func, "solve", 1); */
  if (!(HasQuiet != UNSEEN)) {
    fprintf(
      stderr,
      "c %u variables, %u clauses\n",
  ...
  if (HasCountSol == SEEN_PROCESSED) {
    ...
    fprintf(stderr, "\n");
    int count = funcsatSolCount(func, assumptions, &lastSolution);
  ...
  if (!(HasQuiet != UNSEEN)) {
    fprintf(stderr, "c each row is stats from roughly %u seconds\n",
...
static void deltaStatsHandler(int sig)
{
  ...
  if ((cnt % 100) == 0) {
    fprintf(stderr,
            ...
            "   rest facts   pct (row=%us)\n",
            (AlarmInterval*AlarmNewlineInterval)-1);
    n = 0;
  } else if (((unsigned) cnt % AlarmNewlineInterval) == 0) {
    fprintf(stderr, "\n");
    n = 0;
  }
  ...
  n = fprintf(
    stderr,
    "c %10" PRIu64 " %11" PRIu64 " %10" PRIu64 " %10" PRIu64 " %6" PRIu64 " %5" PRIu64 "   %5.4lf",
